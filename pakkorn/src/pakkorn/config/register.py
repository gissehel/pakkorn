...
def register_key( name, keytype, doc, default=None, internal=False, advanced=False, never_save=False ) :
...
def from_list_to_str(list_value) :
    return list_value
def from_str_to_list(str_value) :
    if type(str_value) == type([]) :
        return str_value
    if (str_value[:1] == '[') and (str_value[-1:] == ']') :
        str_value = str_value[1:-1]
    return str_value.split(',')

register_type( list, from_type=from_list_to_str, from_str=from_str_to_list, ihm_name='list', default=[] )
register_key('',list,doc='', default=[], internal=True, advanced=True )
